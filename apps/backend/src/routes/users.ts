...
export const usersRoute = new Hono()
    ...
    async (c) => {
      try {
        ...
        if (
          ...
          typeof body.profileImage != "undefined"
        ) {
          await uploadProfileImage(
            userData?.profileImage?.id as string,
            userId,
            body.profileImage,
          );
        }
        ...
        if (Object.keys(newUserData).length >= 1) {
          ...
          if (!res) {
            throw Error(
              `Something went wrong when updating user with id: ${userId}`,
            );
          }
        }
  ...
  .post("/default_profile_pict/:userId", async (c) => {
    try {
      const { userId } = c.req.param();

      const image = await readFile(
        path.resolve(process.cwd(), "./src/assets/default_profile_image.png"),
      );
