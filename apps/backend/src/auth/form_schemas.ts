...
export const editMemberNick = z.object({
  changedById: z.string().min(1).max(32).trim(),
...
export const addMemberToConversation = z.object({
  ...
  newUserIds: z.array(z.string().min(1).max(32).trim()).min(1),
});

export const leaveConversation = z.object({
  memberId: z.string().min(1).max(32).trim(),
});
